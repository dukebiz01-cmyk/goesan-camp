...
## v5.0.1 수정
- `camp_events` RLS로 인해 행사 데이터가 실제로 있는데 화면에서 안 보이는 문제 대응
- `js/my.js`에서 행사 조회 오류를 화면에 표시
- 회원 캠핑장명과 행사 camp_name을 정규화 비교
- 업체/강사명도 정규화 비교
- 행사 provider/category 보강
- `gca_v5_0_1_camp_events_rls_screen_fix.sql` 먼저 실행 권장
