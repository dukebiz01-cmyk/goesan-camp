import { db, withTimeout, rpc } from "./supabase.js";
import { state, ROLE_LABEL, KAKAO_OPENCHAT_URL, KAKAO_CHANNEL_URL } from "./config.js";
import { $, val, setLoginError, showLoader, toast } from "./utils.js";
import { applyRoute, navigate, allowedPages, defaultPage } from "./router.js";
...
export async function loadMember(user) {
  ...
  if (!member) {
    setLoginError("계정이 승인되지 않았습니다. 관리자에게 가입승인 또는 이메일 연결을 요청하세요.");
    return false;
  }
...
export async function requestSignup() {
  const payload = {
    ...
    p_email: val("signup-email").toLowerCase(),
  ...
  if (!payload.p_name || !payload.p_email) { toast("이름과 이메일은 필수입니다."); return; }
  showLoader(true);
