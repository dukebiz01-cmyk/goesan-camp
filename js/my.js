...
import { $, esc, fmtDate, todayKey, fileSize, showLoader, toast, normalizeKey } from "./utils.js";
...
function categoryLabel(cat) {
  return { A: "공연", B: "먹거리", C: "체험", D: "활동" }[cat] || cat || "기타";
}

function metaFor(program, row = {}) {
  const key = String(program || "").trim();
  const meta = EVENT_META[key] || {};
  return {
    cat: meta.cat || categoryLabel(row.category),
    provider: meta.provider || row.provider_name || row.vendor_name || row.instructor_name || row.provider || "",
  };
}

function myCamp() {
  return state.member?.camp_name || "";
}

function myProvider() {
  return state.member?.provider_name || state.member?.provider || state.member?.name || "";
}
...
async function loadEvents() {
  ...
  state.events = (data || []).map((e) => {
    ...
    const meta = metaFor(program, e);
    return {
      ...e,
      _program: program,
      _catName: meta.cat || categoryLabel(e.category),
  ...
  box.innerHTML = `
    <div class="page-head">
      <div>
        ...
        <div class="page-sub">내 행사 · 보조사업 서류함 · e나라도움 준비</div>
      </div>
    </div>
    <div id="my-content"><div class="card">행사 데이터 불러오는 중...</div></div>
  `;
...
async function renderMy() {
  ...
  if (role === "admin") {
    rows = state.events;
    title = "관리자 MY";
    sub = "전체 행사와 보조사업 서류 현황을 확인합니다.";
  } else if (["provider", "vendor", "instructor"].includes(role)) {
    rows = state.events.filter((e) => providerMatches(e._provider, provider));
    title = `${provider || "업체"} MY`;
    sub = "내 담당 행사와 방문 캠핑장을 확인합니다.";
  } else {
    rows = state.events.filter((e) => campMatches(e.camp_name, camp));
    title = `${camp || "내 캠핑장"} MY`;
    sub = "내 캠핑장 행사 일정과 보조사업 제출서류를 한 곳에서 관리합니다.";
  }
  ...
  $("my-content").innerHTML = `
    ${state.eventsError ? eventErrorBox(state.eventsError) : ""}
    ...
    <div class="card">
      ...
      ${role !== "admin" && rows.length === 0 && !state.eventsError ? matchHelpBox(role, camp, provider) : ""}
    </div>
...
function eventErrorBox(message) {
  return `
    <div class="status err">
      행사 데이터 조회가 막혀 있습니다.<br>
      원인 가능성: camp_events RLS 또는 권한 정책.<br>
      오류: ${esc(message)}
    </div>
  `;
}

function matchHelpBox(role, camp, provider) {
  if (["provider", "vendor", "instructor"].includes(role)) {
    return `<div class="status warn">담당 행사 0건입니다. 업체 계정의 provider_name/name 값이 행사 업체명과 맞는지 확인하세요. 현재 업체명: ${esc(provider || "-")}</div>`;
  }
  return `<div class="status warn">내 캠핑장 행사 0건입니다. members.camp_name 값과 camp_events.camp_name 값이 맞는지 확인하세요. 현재 회원 캠핑장명: ${esc(camp || "-")}</div>`;
}
...
function eventRows(rows) {
  ...
  return rows.slice(0, 40).map((e) => {
    ...
    const qty = Number(e.qty) > 1 ? ` · ${Number(e.qty)}매` : "";
    return `<div class="item">
      <div class="item-title">${esc(fmtDate(e.event_date))} · ${esc(e._program)}</div>
      <div class="item-meta">🏕 ${esc(e.camp_name || "-")} · ${esc(e._catName || "기타")}${qty}</div>
      <div class="chips">
        <button class="secondary small" data-provider="${esc(provider)}">업체정보: ${esc(provider)}</button>
      </div>
    </div>`;
  }).join("");
}
...
function docUploadBox() {
  return `<div class="file-box">
    ...
    <div class="file-actions">
      ...
      <button class="file" id="btn-doc-file">📎 파일 업로드</button>
...
function updateSelected(type) {
  ...
  $("my-doc-selected").innerHTML = files.length
    ? `<b>${type === "camera" ? "사진" : "파일"} ${files.length}개 선택</b><br>${files.slice(0, 4).map(f => esc(f.name)).join("<br>")}`
    : "선택된 파일 없음";
}
