...
async function createNotice() {
  ...
  try {
    ...
    closeModal(); toast("공지 등록 완료"); loadNewsPage();
...
async function createAgenda() {
  ...
  try {
    ...
    if (error) throw error;
    closeModal(); toast("초안 등록 완료"); loadNewsPage();
...
function openStartVotingModal(proposalId, proposalType) {
  ...
  $("btn-confirm-start").addEventListener("click", async () => {
    ...
    try {
      ...
      if (error) throw error;
      closeModal(); toast(`투표 시작 (정회원 ${data.total_voters}명)`); loadNewsPage();
...
function openExtendModal(proposalId) {
  ...
  $("btn-confirm-extend").addEventListener("click", async () => {
    ...
    try {
      ...
      if (error) throw error;
      closeModal(); toast("마감 연장됨"); loadNewsPage();
...
function bindOnce() {
  ...
  document.addEventListener("click", async (e) => {
    ...
    if (noticeDel) {
      ...
      toast("삭제 완료"); loadNewsPage(); return;
    }
    ...
    if (voteBtn) {
      ...
      try {
        ...
        if (error) throw error;
        toast(`${vote === "yes" ? "찬성" : vote === "no" ? "반대" : "기권"} 등록`); loadNewsPage();
